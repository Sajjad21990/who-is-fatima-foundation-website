...
export default function MessageDetailView({ message }: { message: any }) {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <Link href="/admin/messages" className="flex items-center gap-2 text-sm text-gray-500 hover:text-[#E63946] transition-colors mb-4">
            ...
            <Card className="border-none shadow-sm ring-1 ring-gray-100 overflow-hidden">
                <CardHeader className="bg-[#f8f9fa] border-b border-gray-100 p-8">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="space-y-1">
                            <h2 className="text-2xl font-bold text-[#1D3557]">{message.subject}</h2>
                        ...
                        {message.status !== 'read' && (
                            <span className="px-3 py-1 bg-red-50 text-[#E63946] text-xs font-bold rounded-full uppercase tracking-widest border border-red-100 self-start">
                                New Message
                            </span>
                        )}
                    </div>
                ...
                <CardContent className="p-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12 border-b border-gray-50 pb-8">
                        <div className="space-y-4">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-xl bg-blue-50 text-[#1D3557] flex items-center justify-center font-bold">
                                    <User className="w-5 h-5" />
                                </div>
                                <div>
                                    <p className="text-xs text-gray-400 uppercase font-bold tracking-tighter">Sender Name</p>
                                    <p className="font-bold text-[#1D3557]">{message.name}</p>
                                </div>
                            </div>
                        </div>
                        <div className="space-y-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="font-bold text-[#1D3557] break-all">{message.email}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 text-[#1D3557] font-bold">
                            <MessageSquare className="w-5 h-5 text-[#E63946]" />
                            Message Content
                        </div>
                        <div className="p-6 bg-gray-50 rounded-2xl border border-gray-100">
                            <p className="text-[#1D3557] leading-relaxed whitespace-pre-wrap text-lg">
                                {message.message}
                            </p>
                        </div>
                    </div>
